...
- Treat the standalone desktop viewer as a fixed viewport canvas at CSS sizes ≥1024×600. Preserve authored SVG/viewBox, proportions, geometry and typography; pan, zoom and Fit all provide access to the complete diagram, while Diagram notes contains the original cards in one independently scrollable sidebar. Original node and relationship text stays visible at every zoom. Narrow/short windows retain document flow; print includes the full diagram and cards. For handoff, use the viewport, content-reachability and visual-review checks in [Delivery contract](references/delivery-contract.md#viewer-containment-and-readability).
